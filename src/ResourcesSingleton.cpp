...
/*void ResourcesSingleton::SetTexture(const std::string &a_sName, const autoTexture &a_Texture)
{
    if(m_TextureCache.find(a_sName) == m_TextureCache.end()){
        unsigned int t_iUsed = 1;
        m_TextureCache[a_sName] = std::make_pair(a_Texture, t_iUsed);
    }
}*/
...
{
    if(m_TextureCache.find(a_sName) == m_TextureCache.end()){
        std::shared_ptr<autoTexture> t_TmpTexture(new autoTexture);
